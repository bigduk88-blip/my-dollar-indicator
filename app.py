...
if curr <= avg:
    ...
    advice_msg = f"환율이 3년 평균({avg:,.0f}원)보다 저렴합니다.<br>지금이 가장 안전한 기회입니다. 기계적으로 달러를 모으세요."
...
# (2) 신호등 UI (HTML/CSS 조합)
# 상태에 따라 클래스에 'active' 추가
...
# (3) 박종훈 기자 조언 (상태에 따라 메시지 자동 변경)
if status == "GREEN":
    st.success(f"### {advice_title}\n\n{advice_msg}", icon="✅")
elif status == "YELLOW":
    st.warning(f"### {advice_title}\n\n{advice_msg}", icon="🟡")
else:
    st.error(f"### {advice_title}\n\n{advice_msg}", icon="🚨")
